...
function getNode(data, layer, container) {
    ...
    d3.json('https://opendata.epa.gov.tw/api/v1/ATM00625?%24orderby=county&%24skip=0&%24top=1000&%24format=json', function (data) {
        ...
        svg
            ...
            .attr('width', function (d) {  return d.x1 - d.x0!=0?d.x1 - d.x0: 1})
            .attr('height', function (d) { return d.y1 - d.y0!=0?d.y1 - d.y0:10 })
            .style('stroke', 'red')
            //以layer名稱套用顏色
            .style('fill', function (d) { return color(d.parent.data.name) })
            //以數量大小套用透明度
            .style('opacity', function (d) { return opacity(d.data.PM25) })
